function [ L, EGlob, CClosed, ELocClosed, COpen, ELocOpen, nopath ] = graphProperties( varargin )
...
L = nansum(D(:))/(n*(n-1));
% global efficiency
EGlob = (nansum(nansum(1./(D+eye(n)))) - n)/(n*(n-1));
...
ELocClosed = mean(~isnan(ELocSGClosed));
ELocOpen = mean(~isnan(ELocSGOpen));
end
